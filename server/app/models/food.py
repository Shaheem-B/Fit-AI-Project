...
class CustomFoodCreate(BaseModel):
    ...
    sodium: Optional[float] = 0  # defaults to today
...
class DailyFoodLog(BaseModel):
    ...
    # Track daily water intake (milliliters)
    water_ml: float = 0.0
    ...
    @classmethod
    def validate_water_ml(cls, v):
        if v is None:
            return 0.0
        if v < 0:
            raise ValueError("water_ml must be >= 0")
        return float(v)
...
class DailyFoodLogResponse(BaseModel):
    ...
    meals: Dict[str, List[FoodLogEntry]]
    total_macros: Dict[str, float]
    # Include tracked water intake in responses
    water_ml: float
    created_at: datetime
    updated_at: datetime
